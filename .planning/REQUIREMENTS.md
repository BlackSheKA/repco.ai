...
- [ ] **PRIC-04**: New ENUMs created — `subscription_plan` (`free` | `pro`) and `billing_cycle` (`monthly` | `annual`). Old `subscription_tier` ENUM (`monthly`/`quarterly`/`annual`) dropped (hard switch + wipe — no users to migrate). `users.subscription_plan` (default `free`), `users.billing_cycle` (nullable for free, NOT NULL for pro).
- [ ] **PRIC-05**: `handle_new_user` trigger updated — new signups get `subscription_plan='free'` + 250 cr balance + audit row in `credit_transactions` (no `trial_ends_at`); `startFreeTrial` server action deleted
